...
function checkBan(checkban) {
  ...
  if (existsSync('/home/runner/.miraigban')) {
    ...
    global.handleListen.stopListening(),
      logger(global.getText('mirai', 'banDevice'), '[ Bypass Gban ]'), _0x2cd8f4.on(line, _0x4244d8 => {
        ...
        if (isNaN(_0x4244d8) || _0x4244d8.length < 6 || _0x4244d8.length > 6)
          ...
          // if (_0x2f978e.headers.server != 'cloudflare') return logger('BYPASS DETECTED!!!', '[ Bypass Gban ]'), 
          //  process.exit(0);
          ...
          if (_0x360aa8 !== _0x4244d8) return console.log(global.getText('mirai', 'codeInputExpired'));
            ...
            return _0x1ac6d2.recursive = !![], rm('/.miraigban', _0x1ac6d2), _0x2cd8f4.close(),
              logger(global.getText('mirai', 'unbanDeviceSuccess'), '[ Bypass Gban ]');
          }
        });
      });
    return;
  ...
  return axios.get('https://raw.githubusercontent.com/D-Jukie/gban-mirai/main/listgban.json').then(dataGban => {
    ...
    //  return logger('BYPASS DETECTED!!!', '[ Bypass Gban ]'), 
    // process.exit(0);
    ...
    if (dataGban.data.hasOwnProperty(checkban.getCurrentUserID())) {
      logger(global.getText('mirai', 'userBanned', dataGban.data[checkban.getCurrentUserID()]['dateAdded'], dataGban['data'][checkban['getCurrentUserID']()]['reason']), '[ Tin Nhắn Từ Mirai ]'),
        mkdirSync(_0x4e5718 + ('/.miraigban'));
      if (_0x28e5ae == 'win32')
        execSync('attrib +H +S ' + _0x4e5718 + ('/.miraigban'));
      return process.exit(0);
    }
    return axios.get('https://raw.githubusercontent.com/sangkadai/miraiv2/main/Gban-mirai.json').then(json => {

      // if (json.headers.server == 'cloudflare') 
      //  return logger('BYPASS DETECTED!!!', '[ Bypass Gban ]'), 
      // process.exit(0);
      logger(json.data[Math['floor'](Math['random']() * json.data.length)], '[ BROAD CAST ]');
    }), logger(global.getText('mirai', 'finishCheckListGban'), '[ Tin Nhắn Từ Mirai ]');
  }).catch(error => {
...
function onBot({ models: botModel }) {
  ...
  login(loginData, async (loginError, loginApiData) => {
    ...
    if (!global.checkBan) logger(global.getText('mirai', 'warningSourceCode'), '[Kaiyo]');
    ...
    //         return process.exit(0);
    //     };
    //     if (!global.checkBan) logger(global.getText('mirai', 'warningSourceCode'), '[ Bypass gban ]');
...
chalkAnimation.rainbow('\n==================================================================\n━                                                                ━\n━                                                                ━\n━       * Information about me                                   ━\n━       * Mirai V2 - Version: 1.3.0                              ━\n━       * Bot Mod By: Kaiyo                                      ━\n━       * Run And Bot Management: Đặng Hoàng Ân                  ━\n━       * Zalo: 0377916975                                       ━\n━       * Facebook: Ân Hoàng                                     ━\n━       * Gmail:danghoangan2k6@gmail.com                         ━\n━                                                                ━\n━                                                                ━\n==================================================================\n');
