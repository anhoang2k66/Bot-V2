...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
   ...
   if (args.length == 0) return api.sendMessage(
    ...
    if (permssion < 1) return api.sendMessage("[DONATE]→ Momo/Mbbank: 0377916975. Xin cám ơn ạ!! ❤️", threadID, messageID);
    ...
    if (!find) return api.sendMessage(`[] => Bạn và bot cần là quản trị viên!`,event.threadID);
      ...
      if (permssion != 3) return api.sendMessage("[DONATE]→ Momo/Mbbank: 0377916975. Xin cám ơn ạ!! ❤️", threadID, messageID);
    const find = data.adminIDs.find(el => el.id == event.senderID && api.getCurrentUserID());
    if (!find) return api.sendMessage(`[] => Bạn và bot cần là quản trị viên!`,event.threadID);
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != 100033478361032) return api.sendMessage(`[DONATE]→ Momo/Mbbank: 0377916975. Xin cám ơn ạ!! ❤️`, event.threadID, event.messageID)
            let number = [];
